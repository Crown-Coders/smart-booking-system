...
export default function Services() {
  ...
  const visibleTherapists = useMemo(() => {
    ...
    if (!isAuthenticated) {
      return demoTherapists.filter((therapist) =>
        normalise(therapist.service).includes(normalise(selectedService))
      );
    }

    const exactMatches = therapists.filter((therapist) =>
      normalise(therapist.specialization).includes(normalise(selectedService))
    );

    return exactMatches.length > 0 ? exactMatches : therapists;
  }, [isAuthenticated, selectedService, therapists]);

  const selectedServiceMeta = services.find((service) => service.name === selectedService);
    ...
    <div className="services-page">
      ...
      <section className="services-shell">
        <div className="services-shell__header">
          ...
          <div>
            <h2>{selectedService || "Browse Services"}</h2>
            <p>
        ...
        {selectedService && (
          <div className="therapists-panel">
            <div className="therapists-panel__summary">
              ...
              ) : selectedService && visibleTherapists.length === therapists.length && therapists.length > 0 ? (
                <span>Showing all therapists because no exact specialty match was found.</span>
              ) : null}
            </div>
            ...
            {!loading && visibleTherapists.length === 0 ? (
              <div className="services-empty">
                ...
                <p>
                  {isAuthenticated
                    ? "Once therapists are added by admin, they will appear here for booking."
                ...
                {visibleTherapists.map((therapist) => (
                  <article key={therapist.id} className="therapist-card">
                    ...
                    <div className="therapist-card__body">
                      ...
                      <p className="therapist-card__specialty">{therapist.specialization || selectedService}</p>
